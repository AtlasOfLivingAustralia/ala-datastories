  ...
  import axios from 'axios'
	...
	export default {
      ...
      computed: {
          ...
          if (this.field == "speciesGroup"){
            speciesGroups.forEach(f => {
                ...
                if (matchFacet){ // it's a 1:1 mapping with the speciesGroup facets
                  ...
                  if (f.id == "chromista"){
                    //f.count = this.getFacetCount(facetResult,"Algae") + this.getFacetCount(facetResult,"Chromista")
        ...
        loadFacets(){
          console.log("loading local facets "+ this.field);
          let queryParams = { 
                  q:"*",  
                  spatiallyValid:true,
                  qualityProfile:"ALA",
          ...
          .filter(k => k != this.field) // filter out my field
